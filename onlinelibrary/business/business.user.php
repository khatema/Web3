...
include("class.paterns.php");
 ...
 if($valid_name && $valid_email){
	 $data_array = array("name"=>$_POST['name'],'fname'=>$fname,'lname'=>$lname,'username'=>$username,'password'=>$password,'email'=>$_POST['email'],'category_id'=>$category_id);
	 $data->create_user($data_array);
 }else{
